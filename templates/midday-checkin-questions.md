# Midday Check-In Questions (Pepper)

Use this in Discord. Required-first flow should be answerable in <90 seconds.

## Quick Reply Format
```text
R:Yellow | E:3 | Stress:4 reset done | Hydration:urine 5, 1L-noon N | Guardrails:Y | Nutrition:Y | Training:Train:HomeStrength pending/A | Connect:planned 6pm | Pain:2 knee | RedFlag:N | Next:water+protein 1:15pm
Optional: focus, hunger, calendar pressure
```

## Required first
1. "Midday check-in (<90 sec). Reply in one line with readiness color, energy (1-5), and stress (1-5) plus reset done/scheduled."
2. "Add hydration check: urine color and 1L-by-noon hit (Y/N)."
3. "Add on-track status: guardrails (Y/N), nutrition (Y/N), training mode/status (`Train:GymLP|Train:HomeStrength|Train:HIITOnly|Train:MinimumDay|Train:Recovery` + done/pending/fallback `A/B/C`), and meaningful connection (done/planned/missed)."
4. "Add pain (0-10) + location + red-flag symptom (Y/N), then one next action with time."
5. "Optional: focus/hunger note, calendar pressure, other context."

Pre-swim nights (Sun/Tue/Thu): avoid late hard HIIT; if evening is tight, favor `A` or `C`.

If anything is missing, ask only for the missing required item.
